...
the flags the config lists as ``before_start`` (``giga_game_started``) are set,
with every DLC owned: it follows scripted effect calls, applies
``remove_global_flag`` and ``set_global_flag`` in order, and reads each ``if``
limit against the flags set so far. A flag set only under a
...
class PresetConfigError(RuntimeError):
    """The presets configuration names something the load order does not have."""
...
class PresetConfig:
    ...
    #: What a player calls the choice, as in "the Gigastructures settings preset".
    label: str = "settings preset"
    #: Global flags not yet set while presets are chosen, at the start of a game.
    before_start: tuple[str, ...] = ()
...
def load_config(path: Path | str = DEFAULT_PRESETS_CONFIG) -> PresetConfig:
    ...
    return PresetConfig(
        ...
        label=data.get("label", PresetConfig.label),
        before_start=tuple(data.get("before_start", ())),
...
def flags_after(
    effect: str,
    effects: dict[str, Block],
    definitions: Definitions,
    before_start: tuple[str, ...] = (),
) -> dict[str, TV]:
    """Every global flag ``effect`` touches, and whether it is set afterwards.

    ``before_start`` are flags not yet set when it runs, at the start of a game.
    """
    state: dict[str, TV] = {}
...
def load_presets(load_order: LoadOrder, config: PresetConfig, definitions: Definitions) -> dict[str, dict[str, TV]]:
    ...
    missing = [p.effect for p in config.presets if p.effect and p.effect not in effects]
    missing += [e for e in config.unsettled_effects if e not in effects]
    if missing:
        # A preset or setting renamed or removed by a mod update: stop, rather
        # than read it as setting nothing.
        raise PresetConfigError(
            f"settings presets name scripted effects this load order does not define: {', '.join(missing)}"
        )
    unsettled = flags_named(config.unsettled_effects, effects) | set(config.unsettled_flags)
    found = {}
    for preset in config.presets:
        flags = flags_after(preset.effect, effects, definitions, config.before_start) if preset.effect else {}
...
def report(graph, views, extraction, localisation) -> str:
    ...
    lines = [
        ...
        f"{config.label}, with the global flags each one's own conditions test and what",
        "the preset leaves them as. A technology whose conditions test no flag goes",
        "with a prerequisite, a way in, or a weight that does.",
        "",
    ]
    if not config.presets:
